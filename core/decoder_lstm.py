...
class DecoderLSTM(nn.Module):
    ...
    def forward(self, context, midis_idx, h0=None, c0=None):
        ...
        if h0 is None and c0 is None:
            h0 = context.repeat(self.layer_dim, 1, 1).contiguous() #torch.zeros(self.layer_dim, context.size(0), self.hidden_state)
            c0 = context.repeat(self.layer_dim, 1, 1).contiguous() #torch.zeros(self.layer_dim, context.size(0), self.hidden_state)
